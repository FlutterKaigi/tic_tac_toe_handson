...
1. 状態遷移イベント  
   Board ウィジェットは、  
   初期状態や状態遷移（TicTacToe モデルの差し替え）ごとの setState 実行により、  
   ウィジェット構築 build （状態遷移イベント）を実行します。  
   　  
   ウィジェットツリーが構築/再構築されると、新しい盤面が表示されるほか、  
   _statusMessage(ticTacToe) により、  
   新しい対局状態値からプレーヤーへの指し手依頼や  
   勝敗判定のメッセージが返され、新しいメッセージが表示されます。  
...
2. 指し手（盤面への駒配置）イベント  
   ...
   各セルごとの onTap ハンドラにより、  
   setState メソッドから index 番号から作られた  
   row と col 値に対応するマーク（Ｘ Ｏ または空文字）より、  
   指し手（駒の配置）が有効か否かを判定して、  
   有効であれば対局状態の差し替え（状態遷移）が発生します。  
...
3. ゲームリセット・イベント  
   ...
   setState メソッドから 対局状態モデル TicTacToe の resetBoard() が実行され、  
   新しい対局用の状態に差し替えられる、状態遷移が発生します。  
